	...
	public class ProductosSql
	{
		...
		public static List<Productos> Buscar(int prId)
		{
			...
			while (_reader.Read())
			{
				...
				prProductos.PRECIO_N = _reader.GetDouble(3);
				prProductos.PRECIO_V = _reader.GetDouble(4);
		...
		public static int AgregarVentas(Clases.Ventas v)
		{
			Productos prProductos = new Productos();
			...
			MySqlCommand comando = new MySqlCommand(string.Format(
				"Insert into ventas (ID_USUARIO, ID_PRODUCTO) values ('{0}','{1}')",
				prProductos.Id,pUsuarios.Id),conexion.ObtenerConexion());
			retorno = comando.ExecuteNonQuery();
			return  retorno;
		}
	}
